...
import { Trash2, Plus, Save, GripVertical, Edit, X } from 'lucide-react';
      ...
      <div className="pt-20 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          <div className="lg:col-span-1 bg-white/5 rounded-xl p-6 border border-white/10">
            <h2 className="text-2xl font-bold mb-6">{editingId ? 'Edit Post' : 'Add New Post'}</h2>
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={editingId ? handleUpdatePost : handleAddPost} className="flex-1 bg-white text-black py-2 rounded-lg hover:bg-white/90 font-semibold flex items-center justify-center gap-2">
                  {editingId ? '✏️ Update Post' : (<><Plus className="w-4 h-4" /> Add Post</>)}
                </button>
                {editingId && (
                  <button onClick={handleCancelEdit} className="flex-1 bg-gray-600 text-white py-2 rounded-lg hover:bg-gray-700 font-semibold">
                    Cancel
                  </button>
                )}
              </div>
              ...
              {posts.length === 0 ? (
                ...
                posts.map((post) => (
                  <div key={post.id} draggable onDragStart={() => handleDragStart(post.id)} onDragOver={handleDragOver} onDrop={() => handleDrop(post.id)} className="flex items-center gap-3 p-3 bg-white/10 rounded-lg hover:bg-white/15 cursor-move transition-colors">
                    ...
                    <button onClick={() => handleEditPost(post)} className="text-blue-400 hover:text-blue-300 flex-shrink-0">
